#!/usr/bin/env node
/**
 * Translate specific articles (the 4 new AI agent articles)
 */
...
// Only translate these 4 new articles
const TARGET_SLUGS = [
...
async function translateText(text, langName, context = '') {
  ...
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `Translate to ${langName}. Keep markdown formatting, technical terms (MCP, A2A, x402, ERC-8004, LangChain, CrewAI, AutoGen, etc.), and proper nouns unchanged.
...
async function main() {
  ...
  console.log(`\n📚 Translating ${TARGET_SLUGS.length} new articles\n`);
