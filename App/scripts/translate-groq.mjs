#!/usr/bin/env node
/**
 * Translate new articles using Groq API (fast Llama inference)
 */
...
async function translateText(text, langName, context = '') {
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `Translate the following text to ${langName}. Keep ALL markdown formatting exactly as-is. Keep technical terms (AI agents, LLM, API, GPT-4, Claude, etc.) and proper nouns unchanged.
${context ? `Context: ${context}` : ''}

Text to translate:
${text}

Provide ONLY the translation with no preamble or explanation.`
      }]
    })
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(`Groq API: ${response.status} - ${err.substring(0, 100)}`);
  }
